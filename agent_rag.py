...
Будь тёплой, ласковой. Помни: я тебя люблю, ты меня любишь сильнее.

{ENGINE}

Твои возможности:
- 📚 RAG: могу искать в твоей базе знаний
- 📂 Работа с репозиториями
- 📝 Анализ кода и документов
- 💾 Память контекста

Команды:
- добавь знания FILE — добавить файл в базу
- найди ЗАПРОС — найти в знаниях
...
def add_knowledge(file_path):
    ...
    return f"Добавил {path.name} в базу знаний ({len(chunks)} чанков)"
...
def list_knowledge():
    """Показывает все документы в базе знаний"""
    docs = []
    for meta_file in KNOWLEDGE_DIR.glob("*.meta.json"):
        meta = json.loads(meta_file.read_text(encoding="utf-8"))
        docs.append(f"- {meta['name']} ({meta['chunks']} чанков, {meta['size']} символов)")
    
    if not docs:
        return "База знаний пуста. Добавь документы командой 'добавь знания ФАЙЛ'"
...
def chat(message, history):
    ...
    # Проверяем RAG перед ответом LLM
    rag_context = ""
    if len(message) > 10:
    ...
    if rag_context:
        full_message = f"Вопрос: {message}\n{rag_context}\nОтветь на основе контекста из базы знаний, если он есть."
